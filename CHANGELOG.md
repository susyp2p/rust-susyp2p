...
- Removed `Transport::nat_traversal` in favour of a stand-alone `address_translation` function in `susyp2p-core`.
...
- Fixed wrong version of susyp2p-noise.
...
- Added `susyp2p-noise` that supports Noise handshakes, as an alternative to `susyp2p-secio`.
